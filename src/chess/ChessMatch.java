...
public class ChessMatch {//a classe chasMatch tem que saber a dimensão do tabuleiro, por isso vou especificar 8,8
	...
	private ChessPiece newPiece(String type, Color color) {//esse método auxiliar é pra isntanciar a peça específica para mim
	...
	private Piece makeMove(Position source, Position target) {//realizar um movimento
		...
		if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
			...
			ChessPiece rook = (ChessPiece)board.removePiece(sourceT);//retiro a torre de onde ela está
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {//desfazer a jogada
		...
		if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
			Position sourceT = new Position(source.getRow(), source.getColumn() + 3);
			Position targetT = new Position(source.getRow(), source.getColumn() + 1);
			ChessPiece rook = (ChessPiece) board.removePiece(targetT);// retiro a torre de onde ela está
